...
def func(t, r, W, d, G, g, sigma, t0, W0):
    r = np.asarray(r, dtype=np.complex128)
    drdt = np.zeros_like(r, dtype=np.complex128)
    W[0] = W0[0] * np.exp(-(((t - t0[0]) / (2 * sigma[0])) ** 2))  # Use np.exp instead of np.e **
    W[1] = W0[1] * np.exp(-(((t - t0[1]) / (2 * sigma[1])) ** 2))  # Use np.exp instead of np.e **
...
g = np.zeros(6)
g[1] = 0.5  # Make sure g[0] is defined (I assumed it should be 0.5)
g[2] = 0.5  # Make sure g[0] is defined (I assumed it should be 0.5)
...
plt.plot(sol.t, np.real(r11), label="Re(r11)")
plt.plot(sol.t, np.real(r22), label="Re(r22)")
plt.plot(sol.t, np.real(r33), label="Re(r33)")
